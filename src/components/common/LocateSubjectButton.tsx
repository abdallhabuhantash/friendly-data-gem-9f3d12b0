...
import { expectedSubjectLabel, locateSearch, locateTargetFor } from "@/lib/subject-locate";
  ...
  return (
    <Button asChild size="sm" variant="outline" className={className ?? "h-7 px-2 text-[11px]"}>
      <Link to="/monitoring" search={locateSearch(target)}>
        <Crosshair className="size-3" /> Locate {expectedSubjectLabel(target.subjectNumber)}
      </Link>
    </Button>
  );
}
